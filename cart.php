    ...
    <div class="box-container">
        ...
        if (mysqli_num_rows($select_cart) > 0) {
            while ($fetch_cart = mysqli_fetch_assoc($select_cart)) {
                ...
                $stock_query =  mysqli_query($conn, "SELECT `stock` FROM `products` WHERE id = '$pid'") or die('query failed');
                $stock_row = mysqli_fetch_assoc($stock_query);
                $current_stock = (int)$stock_row['stock'];

                // Check if stock is less than cart_quantity before displaying the form
                if ($current_stock >= $cart_quantity) {
                    $sub_total = $fetch_cart['price'] * $cart_quantity;
                    $grand_total += $sub_total;
                    $final_quantity = $cart_quantity;
                } else {
                    // Allow client to update in max of stock
                    $sub_total = $fetch_cart['price'] * $current_stock;
                    $grand_total += $sub_total;
                    $final_quantity = $current_stock;
                }
        ?>
                <div class="box">
                    <a href="cart.php?delete=<?php echo $fetch_cart['id']; ?>" class="fas fa-times" onclick="return confirm('Delete this from cart?');"></a>
                    <a href="view_page.php?pid=<?php echo $pid; ?>" class="fas fa-eye"></a>
                    <img src="uploaded_img/<?php echo $fetch_cart['image']; ?>" alt="" class="image">
                    <div class="name"><?php echo $fetch_cart['name']; ?></div>
                    <div class="price">₱<?php echo $fetch_cart['price']; ?>/-</div>
                    <div class="stock">Available Stock:<?php echo $fetch_cart['stock']; ?></div>
                    <form action="" method="post">
                        <input type="hidden" value="<?php echo $fetch_cart['id']; ?>" name="cart_id">
                        <input type="number" min="1" value="<?php echo $final_quantity; ?>" name="cart_quantity" class="qty">
                        <input type="submit" value="Update" class="option-btn" name="update_quantity">
                    </form>
